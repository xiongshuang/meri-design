<template>
	<div class="p-select-area-box">
		<div class="p-select-area" :style="{height: height+'px',width: 2 * width + 'px'}">
			<div class="p-select-area-child p-transfer-left" :style="{width: width + 'px'}">
				<section class="p-transfer-left-content" :style="{height: (height)+'px'}" v-show="!searchValue">
					...
					/>
					<!--一维多选列表-->
					<!-- <SelectOptionMultiple v-else :data="mulData" @change="optionChange" /> -->
				</section>
				<section class="p-transfer-left-content" :style="{height: (height)+'px'}" v-show="searchValue">
					<!--搜索数据列表 -s-->
			...
			<div class="p-select-area-child p-transfer-right" :style="{width: width + 'px'}">
    ...
    export default {
        ...
        watch: {
			...
			showChoseData: {
				handler(n, o) {
					if(n !== o) {
						this.selectedData = JSON.parse(JSON.stringify(n))
						this.confirmData = JSON.parse(JSON.stringify(n))
					}
				},
				deep: true
			}
		...
		mounted() {
			this.selectedData = JSON.parse(JSON.stringify(this.showChoseData))
			this.confirmData = JSON.parse(JSON.stringify(this.showChoseData))
		},
        methods: {
            /**
             * 检查两个数组是否一样
