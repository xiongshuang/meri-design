<template>
	<transition :name="position?'selectDownUpExtend':'selectDownUpExtendTop'">
		...
		>
			...
			<div @click="handleHeight" ref="dropDown" class="p-select-drop-down-tree" v-show="!notFound">
	...
	watch: {
		dropDownShow: {
			handler(n, o) {
				if(n) {
					...
					this.handleHeight()
					this.templateTreeData = JSON.parse(JSON.stringify(this.treeData))
				}
			},
			immediate: true
		},
		inputValue(n, o) {
			if(n !== o) {
				let _this = this
				_this.handleHeight()
				_this.optionData = _this.searchData.map(item => {
					let _index = _this.choiceData.findIndex(selectItem => item.lId === selectItem.id)
					if(_index !== -1) {
						item.selected = true
					} else {
						item.selected = false
	...
	methods: {
		...
		handleHeight() {
			this.$nextTick(() => {
				...
				if (height > this.height - 46) target = true
				else target = false
		...
		handleConfirm() {
			...
			this.dropDownShow=false;
			this.$emit('change', this.choiceData);
		},
		// 取消已选择的数据
		cancelTreeData() {
			this.treeData = JSON.parse(JSON.stringify(this.templateTreeData))
		},
		/**
		 * 通过子父项id查找子项id
		 * @param result 结果id
