...
const SetDaysArrayDisabled=(scope, arr) => {
    if (scope) {
        const reg=/[.]/g, [s, e]=scope.split('-');
        const scopeS=s.replace(reg, ''),
...
const SetMonthsArrayDisabled=(scope, arr) => {
    if (scope) {
        const reg=/[.]/g, [s, e]=scope.split('-');
        const scopeS=s.replace(reg, '').substr(0, 6),
...
export { SetDaysArrayDisabled, SetWeeksArrayDisabled, SetMonthsArrayDisabled, SetYearsArrayDisabled };