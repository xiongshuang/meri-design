<template>
  <transition :name="position ? 'selectDownUpExtend': 'selectDownUpExtendTop'" v-if="dropDownListShow">
    <div v-show="dropDownShow" @mouseleave="buttonLeave" @mouseenter="buttonEnter"
         @blur="DropListBlur" @focus="DropListFocus" :tabindex="-1"
         class="drop-list-surround" :class="`drop-list-${type} drop-list-${size} ${disabledItem ? 'disable-element' : ''} drop-list-padding-${position ? 'top' : 'btm'}`"
